...
for i in new_size:
    #image re-sizing
    image_resized =np.array([resize(k, (int(digits.images[0].shape[0]*i),int(digits.images[0].shape[1]*i)),anti_aliasing=True)  for k in digits.images])
    
    print("---------------------------------------------------------------------")
    print("New resized image shape: ",image_resized.shape,"\n")
    # flatten the images
    ...
    for p in gamma:
        for q in c_value:
            ...
            parameter = "Gamma: {}, C: {}".format(p,q)
            results.append({'image Size Fraction':i,'Hyper parameter':parameter,'Train Accuracy':acc_train,'Dev Accuracy': acc_val, 'Test Accuracy' : acc_test})

            print("[ Gamma: {}, C: {} ] ===> Dev Accuracy : {} , Test Accuracy : {} ".format(p,q,acc_val,acc_test))
            if acc_test>best_acc:
                best_acc=acc_test
                best_params={"Gamma":p ,"C":q}
                val_test_acc={"val_accuract: ":acc_val, "Test_accuracy: ":acc_test}
                best_result = [{'image Size Fraction':i,'Hyper parameter':"Gamma: {}, C: {}".format(p,q),'Train Accuracy':acc_train,'Dev Accuracy': acc_val, 'Test Accuracy' : best_acc}]
